...
class Arguments:
  ...
  cache_dir: str = dataclasses.field(
      metadata={"help": "Path to the cache dir"},
  )
...
def main() -> None:
  ...
  tokenizer = transformers.AutoTokenizer.from_pretrained(args.tokenizer, cache_dir = args.cache_dir)

  model = models.MT5ForRegression.from_pretrained(args.model_name_or_path, cache_dir = args.cache_dir)
  model.to(device)
  model.eval()
